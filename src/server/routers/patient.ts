	...
	getDashboardStats: protectedProcedure
		...
		.query(async ({ ctx, input }) => {
			try {
				...
				const [patientAppointments, availableDoctors] = await ctx.db.$transaction(async prisma => {
			...
			try {
				...
				await ctx.db.patient.update({
					data: {
						...
						dateOfBirth: patientData.dateOfBirth ? new Date(patientData.dateOfBirth) : undefined, // undefined means don't update this field if not provided
						email: patientData.email,
				...
				throw new TRPCError({
					cause: error,
					code: 'INTERNAL_SERVER_ERROR',
					message: error instanceof Error ? error.message : 'Failed to update patient info',
				})
			}
		}),
})
