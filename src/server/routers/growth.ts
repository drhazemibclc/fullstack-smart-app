  ...
  createGrowthMeasurement: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        const patientGender =
          patient.gender.toLowerCase() === 'male' ? 'boys' : 'girls';
  ...
  getGrowthMeasurementsByPatientId: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      try {
        ...
        console.error(
          'Error fetching growth measurements by patient ID:',
          error
        );
        if (error instanceof TRPCError) {
          throw error;
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        const currentWeightKg =
          updateData.weightKg ?? existingMeasurement.weightKg ?? 0;
        const currentHeightCm =
          updateData.heightCm ?? existingMeasurement.heightCm ?? 0;
        ...
        const patientGender =
          existingMeasurement.patient.gender.toLowerCase() === 'male'
            ? 'boys'
            : 'girls';
        ...
        const updatedBmi =
          currentHeightCm > 0
            ? currentWeightKg /
              ((currentHeightCm / 100) * (currentHeightCm / 100))
