...
const protectedAdminProcedure = protectedProcedure.use(
  async ({ ctx, next }) => {
...
export const adminRouter = createTRPCRouter({
  // ✅ Add New Service
  addNewService: protectedAdminProcedure
    .input(ServiceSchema)
    ...
    .mutation(async ({ input }) => {
      ...
      try {
        ...
        throw new TRPCError({
          ...
          code: 'INTERNAL_SERVER_ERROR',
          message:
            error instanceof Error ? error.message : 'Failed to add new doctor',
        });
      }
    }),
    ...
    .mutation(async ({ input }) => {
      ...
      try {
        ...
        throw new TRPCError({
          ...
          code: 'INTERNAL_SERVER_ERROR',
          message:
            error instanceof Error ? error.message : 'Failed to add new staff',
        });
      }
    }),
});
