...
export const billingRouter = createTRPCRouter({
	...
	// ✅ Add new bill
	addNewBill: protectedProcedure.input(PatientBillSchema).mutation(async ({ input }) => {
		try {
	...
	generateBill: protectedProcedure.input(PaymentSchema).mutation(async ({ input }) => {
		try {
			...
			const discountAmount = (Number(validated.discount) / 100) * Number(validated.totalAmount)
