// Import Prisma types
import { type Staff, Prisma } from '@prisma/client'; // Ensure you import Prisma for types
import { TRPCError } from '@trpc/server';
import { z } from 'zod';

import { createTRPCRouter, protectedProcedure } from '@/server/trpc'; // Adjust path as needed
...
const buildStaffSearchWhereClause = (
  search?: string
  ...
  getAllStaff: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      try {
        ...
        const [staffData, totalRecordsCount] = await ctx.db.$transaction(
          async (prisma) => {
